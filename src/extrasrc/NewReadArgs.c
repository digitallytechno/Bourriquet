			...
			if(WBStartup != NULL)
				{
					...
					for(wbarg = WBStartup->sm_ArgList, num = 0L; num < WBStartup->sm_NumArgs; num++, wbarg++)
						{
																	...
																	/*else if(!IsArg("WINDOW", src))
																		{
																			if((i = strlen(src) - 7) > 0)
																				{
																					if((ToolWindow = nrdargs->ToolWindow = AllocVecPooled(pool, i+1)) != NULL) strlcpy(ToolWindow, &src[7], i+1);
																				}
																			else ToolWindow = (STRPTR)"CON:";
																		}  */
																}
															tarray++;
														}
							...
							if(nrdargs->RDArgs->RDA_Source.CS_Buffer != NULL)
								...
								{
									E(DBF_STARTUP, "allocation RDA_Source.CS_Buffer(%ld) erronée", nrdargs->RDArgs->RDA_Source.CS_Length);
									RETURN(ERROR_NO_FREE_STORE);
									return(ERROR_NO_FREE_STORE);
								}
						}
					...
					if((nrdargs->FreeArgs = ReadArgs(nrdargs->Template, (APTR)nrdargs->Parameters, nrdargs->RDArgs)) == NULL)
						{
							...
							E(DBF_STARTUP, "ReadArgs() erroné, IoErr()=%ld", error);
							RETURN(error);
							return(error);
						}
					if(SetSignal(0, SIGBREAKF_CTRL_C) & SIGBREAKF_CTRL_C)
						{
							E(DBF_STARTUP, "ReadArgs() annulé par CTRL-C");
							RETURN(ERROR_BREAK);
							return(ERROR_BREAK);
						}
					...
					if(ToolWindow && WBStartup)
						{
							D(DBF_STARTUP, "WINDOW a été définie");
...
#else
  #warning "NEED_NEWREADARGS manquant ou compilation inutile"
#endif
