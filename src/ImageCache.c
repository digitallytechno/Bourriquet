...
static BOOL LoadImage(struct ImageCacheNode *node)
{
  ...
  if(FileExists(node->filename) == TRUE)
  {
    ...
    // La bitmap source de l'image doit *NE PAS* être libérées automatiquement par la datatypes.library,
...
static enum HashTableOperator DeleteImageCacheNode(UNUSED struct HashTable *table, struct HashEntryHeader *entry, UNUSED ULONG number, UNUSED void *arg)
	{
  		...
  		ENTER();
    	#if defined(DEBUG)
  			if(node->openCount > 0) W(DBF_STARTUP, "  ATTENTION ! openCount de noeuds de cache d'images toujours à %ld!!!", node->openCount);
  		#endif
  		DeleteImage(node);
  		RETURN(htoNext);
  		return htoNext;
