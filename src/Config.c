...
int LoadConfig(struct Config *co, const char *fname)
{
 	int result = -1;
  	...
  	ENTER();
	// Indiquer que cette configuration comme n'étant pas correctement sauvegarder dans advanced, juste au cas où quelque chose serait mauvais.
    		...
    		if(getline(&buf, &buflen, fh) >= 3 && strnicmp(buf, "BCO", 3) == 0)
    			{
      				...
      				// Se rappeler la version chargée pour une future référence
      				co->version = version;
      				while(getline(&buf, &buflen, fh) > 0)
      					{
        					...
        					// recherche de \r ou \n et termine la chaîne ici
        					if((p = strpbrk(buf, "\n")) != NULL) *p = '\0';
							// recherche à partir du début du buffer jusqu'à = ou un espace et
							// NULL termine la chaîne plus tôt afin que buf[]
							// ne contienne que l'identifiant deconfiguration
							for(p = buf; *p != '\0' && *p != '=' && !isspace(*p); p++) ; *p = '\0';
							// nous passons en revue nos options potentielles de configuration
							// et vérifions si le nom correspond à celui stocké dans le buf
...
/// IsValidConfig
// Vérifie si les  the required settings have been made
