...
static BOOL Image_Load(struct Data *data, Object *obj)
	{
  		ENTER();
  		if(data->imageLoaded == FALSE && IsStrEmpty(data->id) == FALSE && IsStrEmpty(data->filename) == FALSE)
  			{
    			...
    			if((node = ObtainImage(data->id, data->filename, _screen(obj))) != NULL)
    				{
      					...
      					if(node->dt_obj != NULL) data->imageLoaded = TRUE;
    				}
  			}
  		...
  		if(data->imageLoaded == TRUE)
  			{
    			...
    			if(IsStrEmpty(data->id) == FALSE) ReleaseImage(data->id, FALSE);
...
static void Image_Scale(struct Data *data)
	{
  		...
  		if(data->imageNode.bitmap != NULL && data->imageNode.depth > 0)
  			{
    			...
    			if((scaleHeightDiff > 0 && data->maxHeight > 0) || (scaleWidthDiff > 0 && data->maxWidth > 0))
    				{
      					ULONG newWidth;
      					ULONG newHeight;
      ...
      // have to use the PDTM_SCALE method of picture.datatype v45
            ...
            // for writing the image data of the image in our pixelArray
            									result = DoMethod(data->imageNode.dt_obj, PDTM_READPIXELARRAY, data->scaledPixelArray, 
																					data->imageNode.pixelFormat, 
																					data->scaledBytesPerRow,
                                                                           			0, 0, newWidth, newHeight);

            									if(result == FALSE)
            										{
              											W(DBF_IMAGE, "ATTENTION ! PDTM_READPIXELARRAY sur l'image '%s' avec une profondeur de %ld a échoué après PDTM_SCALE!", data->imageNode.id, data->imageNode.depth);
              											FreeVecPooled(G->SharedMemPool, data->scaledPixelArray);
              											data->scaledPixelArray = NULL;
            										}
            									else
            										{
              											...
              											D(DBF_IMAGE, "Mise à l'échelle du réseau de pixels dans la zone d'image (w/h) à partir de %ld/%ld vers %ld/%ld", data->imageNode.width,
          ...
          // rescale the image to its original size
        					...
        					else	W(DBF_STARTUP, "ATTENTION ! la méthode PDTM_SCALE sur l'image '%s' retourne une erreur!  picture.datatype >= v45?", data->imageNode.id);
      					}
      // check if we generated a scaled interpretation or not
        ...
        // if required. But we use BitMapScale() for all operations
          ...
          // read out the scaled values
          							...
          							D(DBF_IMAGE, "Mise à l'échelle de l'image dans la zone (w/h) de l'image à partir de %ld/%ld à %ld/%ld", data->imageNode.width,
                                                                                  data->imageNode.height,
                                                                                  data->scaledWidth,
                                                                                  data->scaledHeight);
        						}
        // now we can allocate the new bitmap for our scale mask
        					if(data->imageNode.mask != NULL && (data->scaledBitMask = AllocBitMap(newWidth, newHeight, 1L, BMF_CLEAR|BMF_MINPLANES, NULL)) != NULL)
        						{
          							struct BitScaleArgs args;
          							struct BitMap bm;
				          ...
				          // scale the image now with the arguments set
          							BitMapScale(&args);
							        D(DBF_IMAGE, "Mise à l'échelle du masque de transparence de l'image");
        						}
        					else
        						{
          							D(DBF_IMAGE, "Pas de bitmask pour l'image disponible,  ... ignoré");
          							data->scaledBitMask = NULL;
        						}
      					}
    				}
    				...
    				{
      					D(DBF_IMAGE, "Pas de réduction d'échelle requit pour l'image '%s'", data->imageNode.id);
			      		data->scaledBitMap = NULL;
      					data->scaledBitMask = NULL;
      					data->scaledPixelArray = NULL;
    				}
...
OVERLOAD(OM_NEW)
	{
  		...
  		if((obj = DoSuperNew(cl, obj, TAG_MORE, inittags(msg))) != NULL)
  			{
    			GETDATA;
...
OVERLOAD(OM_SET)
	{
      					...
      					case ATTR(ID):
      						{
        						...
        						if(data->id != NULL)
        							{
          								if(IsStrEmpty(newId) == TRUE)  ReleaseImage(data->id, TRUE);
							          	free(data->id);
          								data->id = NULL;
        							}
      					...
      					case ATTR(Filename):
      						{
        						...
        						if(newFilename != NULL)
        							...
        							{
          								if(data->id != NULL) ReleaseImage(data->id, TRUE);
        							}
						        // make the superMethod call ignore those tags
        						tag->ti_Tag = TAG_IGNORE;
      						}
  		...
  		if(relayout == TRUE && data->setup == TRUE && Image_Load(data, obj) == TRUE) Image_Scale(data);	
  		result = DoSuperMethodA(cl, obj, msg);
  		RETURN(result);
  		return result;
	}
...
OVERLOAD(MUIM_Setup)
	{
  		...
  		if((result = DoSuperMethodA(cl, obj, msg)))
  			{
    			if(Image_Load(data, obj) == TRUE) Image_Scale(data);
	    		data->setup = TRUE;
  			}
  		RETURN(result);
  		return result;
...
OVERLOAD(MUIM_AskMinMax)
	{
  		...
  		if(data->imageLoaded == TRUE)
  			{
    			if(data->scaledBitMap != NULL ||  data->scaledPixelArray != NULL)
    				{
  		...
  		if(data->free_horiz) mi->MaxWidth += MUI_MAXMAX;
  		else mi->MaxWidth += minwidth;
        RETURN(0);
  		return 0;
	}
      					...
      					if(data->scaledBitMap != NULL)
      						{
								...
								if(data->scaledBitMask != NULL)
        							{
          								D(DBF_IMAGE, "Dessin de la bitmap redimensionnée/masquée de l'image '%s' (%s)", data->id, data->filename);
          								// we use an own BltMaskBitMapRastPort() implemenation to also support
          								// interleaved images.
          								MyBltMaskBitMapRastPort(data->scaledBitMap, 0, 0, rp,
                                  													_mleft(obj) + (_mwidth(obj) - width) / 2,
                                  													_mtop(obj) + (_mheight(obj) - data->label_height - height) / 2,
                                  													width, height, (ABC|ABNC|ANBC), data->scaledBitMask->Planes[0]);
        							}
        						else
        							{
        		  						D(DBF_IMAGE, "Dessin de la bitmap redimensionnée de l'image '%s' (%s)", data->id, data->filename);
          								BltBitMapRastPort(data->scaledBitMap, 0, 0, rp,
																	_mleft(obj) + (_mwidth(obj) - width) / 2,
																	_mtop(obj) + (_mheight(obj) - data->label_height - height) / 2,
      					...
      					else if(data->scaledPixelArray != NULL)
      						{
        						...
        						if(data->imageNode.pixelFormat == PBPAFMT_ARGB)
        							{
									  	D(DBF_IMAGE, "Dessin de l'mage ARGB '%s' (%s) redimensionnée", data->id, data->filename);
									  	WritePixelArrayAlpha(data->scaledPixelArray, 0, 0, data->scaledBytesPerRow, rp,
														   					_mleft(obj) + (_mwidth(obj) - width) / 2,
														   					_mtop(obj) + (_mheight(obj) - data->label_height - height) / 2,
        							...
        							{
									  	D(DBF_IMAGE, "Dessin de l'image RGB '%s' (%s) redimensionnée", data->id, data->filename);
									  	WritePixelArray(data->scaledPixelArray, 0, 0, data->scaledBytesPerRow, rp,  
																	_mleft(obj) + (_mwidth(obj) - width) / 2,
													  				_mtop(obj) + (_mheight(obj) - data->label_height - height) / 2,
        						...
        						if(data->imageNode.pixelArray != NULL)
        							{
          								...
          								if(data->imageNode.pixelFormat == PBPAFMT_ARGB)
          									{
            									D(DBF_IMAGE, "Dessin de l'image ARGB '%s' (%s)", data->id, data->filename);
            									WritePixelArrayAlpha(data->imageNode.pixelArray, 0, 0, data->imageNode.bytesPerRow, rp,
																					 _mleft(obj) + (_mwidth(obj) - data->imageNode.width) / 2,
																					 _mtop(obj) + (_mheight(obj) - data->label_height - data->imageNode.height) / 2,
																					 data->imageNode.width, data->imageNode.height, 0xffffffff);
          									}
          								else
										  	{
												D(DBF_IMAGE, "Dessin de l'image RGB '%s' (%s)", data->id, data->filename);
												WritePixelArray(data->imageNode.pixelArray, 0, 0, data->imageNode.bytesPerRow, rp,
																			_mleft(obj) + (_mwidth(obj) - data->imageNode.width) / 2,
																			_mtop(obj) + (_mheight(obj) - data->label_height - data->imageNode.height) / 2,
																			data->imageNode.width,	data->imageNode.height, RECTFMT_RGB);
										  }
								...
								// blit the bitmap if we retrieved it successfully.
									...
									{  
									  	if(data->imageNode.mask != NULL)
										  	{
												D(DBF_IMAGE, "Dessin de la bitmap masquée de l'image '%s' (%s)", data->id, data->filename);
												// we use an own BltMaskBitMapRastPort() implemenation to also support
												// interleaved images.
												MyBltMaskBitMapRastPort(data->imageNode.bitmap, 0,	0, rp,
																							_mleft(obj) + (_mwidth(obj) - data->imageNode.width)/2,
																							_mtop(obj) + (_mheight(obj) - data->label_height - data->imageNode.height)/2,
          									...
          									{
												D(DBF_IMAGE, "Dessin de la bitmap de l'image '%s' (%s)", data->id, data->filename);
												BltBitMapRastPort(data->imageNode.bitmap, 0, 0, rp,
																  				_mleft(obj) + (_mwidth(obj) - data->imageNode.width)/2,
																  				_mtop(obj) + (_mheight(obj) - data->label_height - data->imageNode.height)/2,
