...
Description: Classe NList pour l'affichage des émissions de fichiers 
************************************************************/
#include "UploadList_cl.h"
...
OVERLOAD(MUIM_NList_Display)
	{
		...
		//struct Server *sv = (struct Server *)ndm->entry;
  		...
  		/*	}
  			...
  			{*/
    			ndm->strings[0] = GetStr(MSG_UL_NAME_USER);
    			ndm->strings[1] = GetStr(MSG_UL_FILE);
    			ndm->strings[2] = GetStr(MSG_UL_SPEED);
				ndm->strings[3] = GetStr(MSG_UL_TRANSFERRED); 
				ndm->strings[4] = GetStr(MSG_UL_EXCEPTED);
				ndm->strings[5] = GetStr(MSG_UL_SEND_TIME);
    			ndm->strings[6] = GetStr(MSG_UL_STATUS);
    			ndm->strings[7] = GetStr(MSG_UL_PARTS_OBTAINED);
  			}
  		RETURN(0);
  		return 0;
	}
...
DECLARE(MakeFormat)
	{
  		static const int defwidth[8] = { -1,-1,-1,-1,-1,-1,-1,-1 };
