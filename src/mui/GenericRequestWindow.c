...
OVERLOAD(OM_NEW)
	{
  		...
  		if((obj = DoSuperNew(cl, obj,
    																		...
    																		TAG_MORE, inittags(msg))) != NULL)
  			{
    			...
    			xset(obj, MUIA_Window_Title, data->titleText != NULL ? data->titleText : "Bourriquet", MUIA_Window_ScreenTitle, CreateScreenTitle(data->screenTitle, sizeof(data->screenTitle), titleText));
      					...
      					for(token = buttons, i = 0; i < numButtons; i++, token = next)
      						{
								...
								if((next = strchr(token, '|')) != NULL) *next++ = '\0';
								if(*token == '*')
									{
								  		active = TRUE;
								  		token++;
									}
        						if((ul = strchr(token, '_')) != NULL) inputEvent = FALSE;
        						...
        						if(button != NULL)
        							{
										...
										else DoMethod(buttonGroup, OM_ADDMEMBER, button);
          								// which result should this button produce?
          								if(i < numButtons - 1) buttonResult = i + 1;
          								else buttonResult = 0;
          								if(inputEvent == TRUE && numButtons == 2)
          									{
            									if(i == 0) DoMethod(obj, MUIM_Notify, MUIA_Window_InputEvent, "y", obj, 2, MUIM_GenericRequestWindow_FinishInput, buttonResult);
            									else if(i == numButtons - 1) DoMethod(obj, MUIM_Notify, MUIA_Window_InputEvent, "n", obj, 2, MUIM_GenericRequestWindow_FinishInput, buttonResult);
...
OVERLOAD(OM_DISPOSE)
	{
  		...
  		ENTER();
  		if(data->buttons != NULL) free(data->buttons);
  		if(data->titleText != NULL) free(data->titleText);
