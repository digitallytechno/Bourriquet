...
static void Terminate(void)
{
	...
	// Libére la sémaphore globale
  	...
  	if(G->ServerMet != NULL)
		{	
    		if(IsServerListEmpty(G->ServerMet) != FALSE)  E(DBF_NET, "Il y a toujours %ld server(s) dans la liste", G->ServerMet->count);
    		DeleteServerList(G->ServerMet);
    		G->ServerMet = NULL;
  		}		
	if(G->ShareFiles != NULL)
		{	
    		if(IsShareListEmpty(G->ShareFiles) != FALSE)  E(DBF_NET, "Il y a toujours %ld fichier(s) à partatger dans la liste", G->ShareFiles->count);
    		DeleteShareList(G->ShareFiles);
    		G->ShareFiles = NULL;
  		}		
...
static void InitAfterLogin(void)
{
	...
	D(DBF_STARTUP, "Chargement de la configuration...");
  	...
  	res = LoadConfig(C, "PROGDIR:config/.config");
  	if(res == 0)Abort(NULL); 
	...
	ParseServerMetFile("PROGDIR:config/server.met");
	...
	D(DBF_STARTUP, "Recherche des fichiers à partager...");
	//SplashProgress("share", 84);
	SearchDirShare(C->ShareDir, NULL);
    LEAVE();
}
	...
	for(active=TRUE;;)
	  {
			...
			if((G->ShareFiles = CreateShareList()) == NULL)
    			{
      				break;
    			}
			...
			list_init(&G->list_hosts);
			list_init(&G->list_shared);
			list_init(&G->list_downloads);
			list_init(&G->list_research);
					...
					if(signals != 0)
						{
							...
							if(isFlagSet(signals, timerSig)) ProcessTimerEvent();
							//if(isFlagSet(signals, appSig)) HandleAppIcon();
...
// a être utilisé dans des situations où nous avons à attendre des événements spécifiques
